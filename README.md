...
### Claude Code
```bash
/plugin marketplace add lfernandoss/harness-kit
...
### GitHub Copilot CLI
```bash
copilot plugin marketplace add lfernandoss/harness-kit
copilot plugin install harness-kit@harness-kit
```

### Gemini CLI
```bash
agy plugin install https://github.com/lfernandoss/harness-kit
```

### Codex CLI
```bash
codex plugin marketplace add https://github.com/lfernandoss/harness-kit
codex plugin add harness-kit@harness-kit
```

---

## 💻 SDK & Web Dashboard — `@lfernandoss/hrns`

The **Harness Kit SDK** provides both a standalone CLI and an embedded **Web Dashboard daemon** with real-time visual telemetry, enabling sovereign TDD execution directly from the browser or terminal.

![HarnessKit SDK Web Dashboard](sdk-web-screenshot.png)

### 🌐 Web Dashboard (Visual Control Cockpit)

The SDK includes a built-in Single Page Application (SPA) served locally by the HTTP daemon:

* **Reactive Phase Timeline:** Real-time visual tracking with orbital loop animations (`⟳ Em Loop TDD`), phase completion badges, and dynamic progress connectors.
* **Interactive Socratic Refinement:** In `deep_thinking` mode, review and customize architectural questions formulated by the Software Architect before generating code.
* **Live SSE Telemetry:** Server-Sent Events stream real-time logs, state transitions, and step-by-step agent executions directly to the terminal console.
* **F5 Resilience & Auto-Recovery:** Restores session state from `localStorage` on page reload without losing active logs or connection to the job.
* **Cost & Token Governance:** Real-time metrics tracking aggregated token consumption and USD costs.
* **Interactive OpenAPI/Swagger:** Explore and trigger REST endpoints directly via `/docs`.
...
```bash
git clone https://github.com/lfernandoss/harness-kit.git
cd harness-kit/sdk
npm install
npm run build
npm install -g .
...
hrns run --scope "Implement JWT authentication with unit tests" --agent antigravity-cli

# Deep thinking mode with Socratic refinement
hrns run --mode deep_thinking --scope "Migrate session architecture to Redis Cluster"

# Fast mode for simple bugfixes
hrns run --mode fast --scope "Fix CPF validation on signup form"
```
...
| :--- | :--- | :--- |
| `GET` | `/` | Web Dashboard SPA interface |
| `GET` | `/docs` | Interactive Swagger / OpenAPI documentation |
| `POST` | `/orchestrator/run` | Enqueue autonomous job with idempotency key (`202 Accepted`) |
| `GET` | `/orchestrator/stream` | Real-time Server-Sent Events (SSE) log channel |
| `GET` | `/orchestrator/status/:id` | Query job status, active phase, and logs |
| `GET` | `/orchestrator/jobs/latest` | Retrieve latest job for auto-recovery |
| `POST` | `/orchestrator/refine/questions` | Generate Socratic questions for architectural refinement |
| `GET` | `/orchestrator/telemetry/tokens` | Query token and cost telemetry |

> 📄 **Guides:**
> - [SDK & Web Dashboard Complete Guide](docs/workflow/SDK-AND-WEB-GUIDE.md)
> - [SDK Architecture & Reference](sdk/README.md)
...
* **Issues & Feedback:** <https://github.com/lfernandoss/harness-kit/issues>
