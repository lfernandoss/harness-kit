...
Autonomous orchestrator SDK for [harness-kit](https://github.com/lfernandoss/harness-kit). Run a single command and the full TDD loop executes unattended — scope in, backlog built, agents delegated, validation scored, memory persisted.
...
> To uninstall: `npm uninstall -g @lfernandoss/hrns`
...
## Web Dashboard & HTTP Daemon

The SDK contains a built-in HTTP daemon with a reactive Single Page Application (SPA) Web UI:

![HarnessKit SDK Web Dashboard](../sdk-web-screenshot.png)

### Features
* **Live SSE Streaming:** Streams phase transitions, agent interactions, and step-by-step logs via Server-Sent Events.
* **Reactive Phase Timeline:** Visual indicators with orbital radar pulse (`⟳ Em Loop TDD`), task progress bars, and completion badges.
* **Socratic Questionnaire Modal:** In `deep_thinking` mode, presents architectural questions formulated by the Software Architect directly in the UI for review and customization.
* **Auto-Recovery (`F5` Resilience):** Persistent job state in `localStorage` allows refreshing the page without losing active execution logs.
* **Built-in Swagger / OpenAPI:** Interactive API explorer at `http://127.0.0.1:3000/docs`.
...
| `POST` | `/orchestrator/run` | Enqueue autonomous job (`202 Accepted`) |
| `GET` | `/orchestrator/stream` | Server-Sent Events log stream (`?jobId=...`) |
| `GET` | `/orchestrator/status/:id` | Check job status and execution history |
| `GET` | `/orchestrator/jobs/latest` | Retrieve latest job for browser auto-recovery |
| `POST` | `/orchestrator/refine/questions` | Generate Socratic architectural questions |
| `GET` | `/orchestrator/telemetry/tokens` | Aggregate token consumption and USD costs |
...
[github.com/lfernandoss/harness-kit](https://github.com/lfernandoss/harness-kit)
