...
This directory (`src/server/`) contains the headless, non-interactive HTTP daemon capability for `@lfernandoss/hrns`. It allows external clients, CI/CD runners, and Docker containers to trigger and monitor autonomous TDD orchestration loops via RESTful APIs.
...
```typescript
import { startHttpServer } from '@lfernandoss/hrns'
