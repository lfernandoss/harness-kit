...
describe('RouteHandlers Integration Tests', () => {
  ...
  it('POST /orchestrator/settings/renew -> 200 OK resetting settings to default', async () => {
    const req = new MockIncomingMessage('/orchestrator/settings/renew', 'POST')
    const res = new MockServerResponse()

    const handlePromise = routeHandlers.handleRequest(req as unknown as IncomingMessage, res as unknown as ServerResponse)
    req.emit('data', Buffer.from(JSON.stringify({ scope: 'local', project: 'backend' })))
    req.emit('end')
    await handlePromise
  ...
  it('DELETE /orchestrator/settings -> 200 OK deleting target settings file', async () => {
    const req = new MockIncomingMessage('/orchestrator/settings', 'DELETE')
    const res = new MockServerResponse()
  ...
  it('POST /orchestrator/settings/validate -> 200 OK with validation diagnostics', async () => {
    const req = new MockIncomingMessage('/orchestrator/settings/validate', 'POST')
    const res = new MockServerResponse()

    const handlePromise = routeHandlers.handleRequest(req as unknown as IncomingMessage, res as unknown as ServerResponse)
    req.emit('data', Buffer.from(JSON.stringify({ settings: { antigravity: { timeoutMs: -10 } } })))
    req.emit('end')
    await handlePromise
