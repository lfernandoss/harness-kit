...
import { JobExecutionRegistry } from './JobExecutionRegistry'
import { EventStreamBroadcaster } from '../../inbound/http/routes/EventStreamHandler'
...
export class JobRunnerService {
  ...
  async executeJob(job: OrchestrationJob): Promise<void> {
    ...
    const broadcaster = EventStreamBroadcaster.getInstance()
    ...
    try {
      ...
      if (action === 'reset') {
        ...
        const specsDir = join(effectivePath, 'docs', 'specs')
      ...
      orchestratorConfig.onPhaseChange = (phase) => {
        broadcaster.broadcast(job.jobId, {
          type: 'phase_change',
          phase,
          timestamp: Date.now(),
          jobId: job.jobId,
      ...
      orchestratorConfig.onInteractiveRefinement = async (questions) => {
        broadcaster.broadcast(job.jobId, {
          type: 'interactive_refinement',
          questions,
          timestamp: Date.now(),
          jobId: job.jobId,
        } as any)
        ...
        broadcaster.broadcast(job.jobId, {
          type: 'log_chunk',
          stream: 'stdout',
          text: `[REFINEMENT] 🧠 ${questions.length} questões socráticas formuladas pelo Software Architect. Aguardando validação interativa...`,
          timestamp: Date.now(),
          jobId: job.jobId,
        })

        return new Promise((resolve) => {
          JobExecutionRegistry.getInstance().registerPendingRefinement(job.jobId, questions, (answers) => {
            broadcaster.broadcast(job.jobId, {
              type: 'log_chunk',
              stream: 'stdout',
              text: `[REFINEMENT] ✅ Respostas recebidas! Consolidando decisões em docs/product/REFINEMENT.md...`,
              timestamp: Date.now(),
              jobId: job.jobId,
            })
            resolve(answers)
      ...
      const executionRegistry = JobExecutionRegistry.getInstance()
      executionRegistry.registerRunning(job.jobId, orchestrator, abortController)
      ...
      broadcaster.broadcast(job.jobId, {
        type: 'log_chunk',
        stream: 'stderr',
        text: `\n[JOB ERROR] ❌ Falha na execução do ciclo: ${message}\n`,
        timestamp: Date.now(),
        jobId: job.jobId,
      })
      await this.jobStore.updateStatus(job.jobId, 'failed', { code, message })
    } finally {
      JobExecutionRegistry.getInstance().unregister(job.jobId)
  ...
  private async prepareWorkspaceGit(
    ...
    if (existsSync(join(workspacePath, '.git'))) {
      // JIT Sync: fetch latest changes from origin for baseBranch (non-blocking, short timeout)
      try {
        await execGit(['fetch', 'origin', baseBranch], workspacePath, 5000)
      } catch {}
    }
    ...
    if (useWorktree && existsSync(join(workspacePath, '.git'))) {
      ...
      if (addRes.exitCode !== 0) {
        addRes = await execGit(['worktree', 'add', '-B', targetBranch, worktreePath], workspacePath, 8000)
      }
      if (addRes.exitCode === 0) {
    ...
    if (existsSync(workspacePath) && existsSync(join(workspacePath, '.git'))) {
