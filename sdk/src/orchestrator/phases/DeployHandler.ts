  ...
  async handle(phase: Phase, context: Reviewontext): Promise<Phase | null> {
    ...
    for (const projectPath of projectPaths) {
      ...
      try {
        ...
        // Push — try pushing upstream, otherwise fallback gracefully without failing entire cycle
        try {
          this.runGit(projectPath, ['push', '-u', 'origin', 'HEAD'])
          process.stdout.write(`  ${AnsiHelpers.dim('git push -u origin HEAD')} ${AnsiHelpers.green('✔')}\n`)
        } catch {
          try {
            this.runGit(projectPath, ['push'])
            process.stdout.write(`  ${AnsiHelpers.dim('git push')} ${AnsiHelpers.green('✔')}\n`)
          } catch (pushErr: any) {
            process.stderr.write(`  ${AnsiHelpers.yellow('⚠')} ${AnsiHelpers.dim('Remote push skipped/unreachable:')} ${pushErr.message}\n`)
          }
        }
