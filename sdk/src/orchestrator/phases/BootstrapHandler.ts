...
export class BootstrapHandler extends AbstractPhaseHandler {
  async handle(phase: Phase, context: Reviewontext): Promise<Phase | null> {
    ...
    if (context.config.action === 'reset') {
      ...
      const devStatePath = join(productDir, 'DEVELOPMENT-STATE.md')
      try {
        const { unlinkSync } = require('node:fs')
        if (existsSync(backlogPath)) unlinkSync(backlogPath)
        if (existsSync(refinementPath)) unlinkSync(refinementPath)
        if (existsSync(questionsPath)) unlinkSync(questionsPath)
        if (existsSync(devStatePath)) unlinkSync(devStatePath)
      } catch {}
    }
    ...
    if (context.config.action !== 'reset' && existing.length > 0) return shouldRefine ? Phase.REFINEMENT : Phase.PLANNING
