...
export abstract class AbstractCliRunner implements IAgentRunner {
  ...
  private spawnAndCollect(
    ...
    return new Promise<AgentOutput>((resolve, reject) => {
      ...
      const binary =
        process.platform === 'win32' && !this.binaryName.includes('.')
          ? `${this.binaryName}.exe`
          : this.binaryName
      ...
      if (this.#timeoutMs > 0 && !options?.signal) {
        timer = setTimeout(() => {
          killProcessGroup()
          const timeoutErr = `${this.binaryName} runner timed out after ${this.#timeoutMs}ms`
          invocation.onLog?.(`[CLI TIMEOUT] ❌ ${timeoutErr}`, 'stderr')
      ...
      child.on('error', (err: NodeJS.ErrnoException) => {
        ...
        const errMsg = err.code === 'ENOENT'
          ? `${this.binaryName} CLI não encontrado no PATH do sistema. Certifique-se de que o executável está instalado.`
          : `${this.binaryName} CLI erro: ${err.message}`
        invocation.onLog?.(`[CLI ERROR] ❌ ${errMsg}`, 'stderr')
        if (DebugContext.enabled) {
      ...
      child.on('close', (code) => {
        ...
        if (code !== 0) {
          ...
          const errLogMsg = `[CLI ERROR] ❌ ${this.binaryName} encerrou com código de erro (${code})${detail}`
          invocation.onLog?.(errLogMsg, 'stderr')
          if (DebugContext.enabled) {
